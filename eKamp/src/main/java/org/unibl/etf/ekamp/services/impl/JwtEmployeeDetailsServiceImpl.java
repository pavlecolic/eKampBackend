...
public class JwtEmployeeDetailsServiceImpl implements JwtEmployeeDetailsService {
    ...
    @Override
    public JwtEmployee loadEmployeeByUsername(String username) throws UsernameNotFoundException {
        return modelMapper.map(employeeEntityRepository.findByUsername(username).orElseThrow(() -> new UsernameNotFoundException(username)), JwtEmployee.class);
    }
}
